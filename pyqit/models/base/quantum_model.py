...
class BaseQuantumModel(BaseModel):
    ...
    def diff_methods(self, X) -> dict:
        ...
        The model's ``diff_method`` when one was named, else what PennyLane
        resolves ``"best"`` to. ``backprop`` and ``adjoint`` are simulator-only;
        a device with shots or real hardware resolves to ``parameter-shift``,
        which costs two circuit executions per parameter for every gradient.
