...
def _is_torch_transform(fn) -> bool:
    if fn is None:
        return False
    module = getattr(type(fn), "__module__", "") or ""
    if module.startswith(("torch", "torchvision")):
        return True
    try:
        for cls in type(fn).__mro__:
            if (getattr(cls, "__module__", "") or "").startswith("torch"):
                return True
    except Exception:
        pass
    return False
...
class _NumpyLoader:
    ...
    def __repr__(self):
        ...
        return f"_NumpyLoader(n={len(self.X)}, batch_size={self.batch_size}, shuffle={self.shuffle}{t})"


def _make_torch_loader(X, y, batch_size, shuffle,
                       num_workers=0,
...
class _Normalizer:
    ...
    def transform(self, X):
        if self.method in ("minmax", "zscore") and not self._fitted:
            raise RuntimeError(f"'{self.method}' normalizer must be fitted before transform().")
        if self.method == "minmax":
...
def _prescale_angle_pi(X, n_qubits):
    n_f = X.shape[1]
    X   = np.hstack([X, np.zeros((len(X), n_qubits - n_f))]) if n_f < n_qubits else X[:, :n_qubits]
...
def _prescale_amplitude(X, n_qubits):
    target = 2 ** n_qubits
    n_f    = X.shape[1]
    X      = np.hstack([X, np.zeros((len(X), target - n_f))]) if n_f < target else X[:, :target]
...
def _prescale_binary(X, n_qubits):
    ...
    n_f = X.shape[1]
    return np.hstack([X, np.zeros((len(X), n_qubits - n_f))]) if n_f < n_qubits else X[:, :n_qubits]
...
class DataModule:
    ...
    def __init__(
        ...
        if len(split) != 3:
            raise ValueError(f"split must be a 3-tuple (train, val, test), got {split!r}.")
    ...
    @classmethod
    def from_dataframe(cls, df: Any, label_col: Union[str, int] = -1,
                       **kw) -> "DataModule":
        col = df.columns[label_col % len(df.columns)] if isinstance(label_col, int) else label_col
    ...
    @classmethod
    def from_csv(cls, path: Union[str, Path], label_col: Union[str, int] = -1,
                 delimiter: str = ",", **kw) -> "DataModule":
    ...
    def setup(
        ...
        if backend    is not None: self._backend   = backend
        if batch_size is not None: self.batch_size = batch_size
        nq = n_qubits or self.n_qubits

        numpy_fns, torch_fns = [], []
        fns = self.transform if isinstance(self.transform, list) else (
        ...
        if torch_fns and self._backend == "pennylane":
            ...
            raise RuntimeError(
                ...
                f"Torch transforms operate on float32 Tensors; the PennyLane backend\n"
                f"works with numpy arrays throughout.\n"
                f"Either:\n"
                f"  • switch to backend='torch'\n"
                f"  • replace with a numpy-compatible callable"
            )
        ...
        if prescale is not None:
            if nq is None:
                raise RuntimeError(
                    "Prescaling requires n_qubits. "
                    "Set it in __init__ or the Trainer supplies it from model.n_qubits."
                )
            X_tr             = _apply_prescale(X_tr, prescale, nq)
        ...
        if self._numpy_transform is not None and self._backend == "torch":
            X_tr             = np.stack([self._numpy_transform(X_tr[i]) for i in range(len(X_tr))])
            X_va = np.stack([self._numpy_transform(X_va[i]) for i in range(len(X_va))]) if X_va is not None else None
            X_te = np.stack([self._numpy_transform(X_te[i]) for i in range(len(X_te))]) if X_te is not None else None
    ...
    def val_loader(self, shuffle: bool = False):
        ...
        return self._make_loader(self._X_val, self._y_val, shuffle) if self._X_val is not None else None

    def test_loader(self, shuffle: bool = False):
        self._assert_setup("test_loader")
        return self._make_loader(self._X_test, self._y_test, shuffle) if self._X_test is not None else None
    ...
    def _make_loader(self, X, y, shuffle):
        ...
        numpy_t = (None if self._numpy_transform_applied_in_setup
                   else self._numpy_transform)
        return _NumpyLoader(X, y, self.batch_size, shuffle, self.seed, numpy_t)


    @property
    def X_train(self): self._assert_setup("X_train"); return self._X_train
    @property
    def y_train(self): self._assert_setup("y_train"); return self._y_train
    @property
    def X_val(self):   self._assert_setup("X_val");   return self._X_val
    @property
    def y_val(self):   self._assert_setup("y_val");   return self._y_val
    @property
    def X_test(self):  self._assert_setup("X_test");  return self._X_test
    @property
    def y_test(self):  self._assert_setup("y_test");  return self._y_test
    ...
    @property
    def n_samples(self):   return len(self.X_raw)
    @property
    def n_features(self):  return self.X_raw.shape[1]
    @property
    def n_classes(self):   return len(np.unique(self.y_raw))
    @property
    def class_labels(self): return np.unique(self.y_raw)
    @property
    ...
    def reconfigure(self, **kwargs) -> "DataModule":
        _ok = {"normalize","encoder","n_qubits","split",
               "stratify","seed","batch_size","num_workers","transform"}
        for k, v in kwargs.items():
            if k not in _ok:
                raise ValueError(f"reconfigure() does not accept {k!r}. Valid: {sorted(_ok)}")
            setattr(self, k, v)
        self._is_setup   = False
        self._normalizer = None
        self._numpy_transform = None
    ...
    def summary(self):
        #TODO: implement a summary method that prints out the configuration and stats of the DataModule
        pass

    def __repr__(self):
        enc = self.encoder.__name__ if self.encoder is not None else "none"
        return (f"DataModule(name={self.name!r}, n={self.n_samples}, "
    ...
    def _do_split(self):
        ...
        if self.stratify:
            ...
            if val_frac > 0 and test_frac > 0:
                X_tr, X_tmp, y_tr, y_tmp = tts(X, y,
                    test_size=val_frac + test_frac,
                    random_state=self.seed, stratify=y)
                X_va, X_te, y_va, y_te = tts(X_tmp, y_tmp,
                    test_size=test_frac / (val_frac + test_frac),
                    random_state=self.seed, stratify=y_tmp)
            elif val_frac > 0:
                X_tr, X_va, y_tr, y_va = tts(X, y, test_size=val_frac,
                    random_state=self.seed, stratify=y)
                X_te = y_te = None
            elif test_frac > 0:
                X_tr, X_te, y_tr, y_te = tts(X, y, test_size=test_frac,
                    random_state=self.seed, stratify=y)
                X_va = y_va = None
            else:
                X_tr, y_tr = X, y
        ...
        return (
            X[i_tr], y[i_tr],
            X[i_va] if len(i_va) else None, y[i_va] if len(i_va) else None,
            X[i_te] if len(i_te) else None, y[i_te] if len(i_te) else None,
        )



def _compose(fns: list) -> Optional[Callable]:
    if not fns:
        return None
    if len(fns) == 1:
        return fns[0]
